...
class Extractor(BaseMosqueWebsiteExtractor):
    key = "masjid_zakariya_65f43dbe"
    version = "2026.06.12.1"
    source_match = SourceMatch(domains=("masjidzakariya.co.uk",))
    refresh_policy = RefreshPolicy(frequency=RunFrequency.DAILY)
    targets = (
        TargetSpec(
            label="timetable",
            url="https://masjidzakariya.co.uk/",
    ...
    def extract(self, ctx: ExtractContext) -> ExtractorResult:
        ...
        # Daily dptTimetable widget: Prayer | Start | Jamaat (today only, no date col)
        ...
        # Jumu'ah cell (may contain 1 or more times, e.g. "1:30 pm | 4:15 pm")
        ...
        if not rows:
            # Common placeholder until admin populates the widget
