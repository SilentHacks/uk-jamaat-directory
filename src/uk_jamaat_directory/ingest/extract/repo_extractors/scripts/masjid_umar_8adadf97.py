...
class Extractor(BaseMosqueWebsiteExtractor):
    key = "masjid_umar_8adadf97"
    version = "2026.06.12.1"
    source_match = SourceMatch(domains=("umarmasjid.co.uk",))
    refresh_policy = RefreshPolicy(frequency=RunFrequency.DAILY)
    targets = (
        TargetSpec(
            label="timetable",
            url="https://umarmasjid.co.uk/",
    ...
    def extract(self, ctx: ExtractContext) -> ExtractorResult:
        ...
        # The plugin renders a daily today-only table with explicit Jamat column.
