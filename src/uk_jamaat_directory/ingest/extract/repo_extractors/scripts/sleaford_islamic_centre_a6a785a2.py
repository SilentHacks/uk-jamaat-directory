...
class Extractor(BaseMosqueWebsiteExtractor):
    key = "sleaford_islamic_centre_a6a785a2"
    version = "2026.06.12.1"
    source_match = SourceMatch(domains=("sleafordislamiccentre.org",))
    refresh_policy = RefreshPolicy(frequency=RunFrequency.DAILY)
    targets = (
        TargetSpec(
            label="timetable",
            url="https://sleafordislamiccentre.org/",
    ...
    def extract(self, ctx: ExtractContext) -> ExtractorResult:
        ...
        # The plugin renders a daily today-only table with explicit Iqamah column.
