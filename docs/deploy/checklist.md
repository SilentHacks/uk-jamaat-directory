...
- [ ] `.env` on the server updated if new settings were added (compare with `.env.vps.example`)
...
3. [ ] **Build** images (`docker compose -f docker-compose.vps.yml build`)
4. [ ] **Migrate** database (`./scripts/deploy/migrate.sh`) — explicit step, never skip on schema changes
5. [ ] **Restart** services (`docker compose -f docker-compose.vps.yml up -d`)
...
1. Stop api/worker/beat: `docker compose -f docker-compose.vps.yml stop api worker beat`
